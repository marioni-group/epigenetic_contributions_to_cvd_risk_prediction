...
#join with cox
W1 = subset(merged, set == "wave1") # 3711 # IQR 3667 # Troponin 3659
W3_W4 = subset(merged, set == "wave4" | set == "wave3") # 9079  #IQR 8966 # Troponin 8998

#keep only unrelated
unrelated = read.csv(settings$prep.unique_W3_W4)
W3_W4 = W3_W4[which(W3_W4$id %in% unrelated$Sample_Name),] #6936 #IQR 6856 # Troponin 6880
...
head(W3_W4[start:15])
W3_W4[start:end] = mclapply(W3_W4[start:end], scale)
head(W3_W4[start:15])
  
W1[start:end] = mclapply(W1[start:end], transform)
W1[start:end] = mclapply(W1[start:end], scale)
