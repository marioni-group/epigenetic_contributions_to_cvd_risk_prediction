...
names(cox) = c("id", "sex", "age", "Troponin_T", "Troponin_I", "cTnI_corrected", "simd", "history_cvd", "diabetic",
               "cigs_day", "pack_years", "sys_bp", "ra", "total_cholesterol", "HDL_cholesterol", "bp_medicines")
...
write.csv(cox, paste0(path, 'Ola/Lab/ASSIGN/GS_assign_data.csv'), row.names = F))

cox_cTnT = subset(cox, !is.na(Troponin_T))
cox_cTnT = cox_cTnT[-c(5,6)]

cox_cTnI = subset(cox, !is.na(Troponin_I))
cox_cTnI = cox_cTnI[-c(4)]

cox_cTnT$cTnT_rnk = outlierTrim(transform(cox_cTnT$Troponin_T))
cox_cTnI$cTnI_rnk = outlierTrim(transform(cox_cTnI$Troponin_I))
cox_cTnI$cTnI_corrected_rnk = outlierTrim(transform(cox_cTnI$cTnI_corrected))

write.csv(cox_cTnT, paste0(path, 'Ola/Lab/ASSIGN/GS_assign_data_cTnT.csv'), row.names = F)
write.csv(cox_cTnI, paste0(path, 'Ola/Lab/ASSIGN/GS_assign_data_cTnI.csv'), row.names = F)
